...
class ConformalScaledModel(Model):
    ...
    def fit(self, train: pd.DataFrame) -> None:
        """Fit the base model, then set the per-window conformal scale so train central-p coverage ~= p; mean is untouched (calibration, not skill)."""
        self.base.fit(train)
    ...
    def predict(self, eval_index: pd.Index) -> ProbForecast:
        """Base forecast with sigma scaled by the fitted conformal factor; mean unchanged."""
