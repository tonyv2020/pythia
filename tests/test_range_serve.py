...
def _wide(n=400):
    ...
    return pd.DataFrame({"QQQ_close": close, "QQQ_high": close+spread,
                         "QQQ_low": close-spread}, index=idx)


def test_range_block_shape_and_positive_cone():
    blk = compute_range_block(_wide(), symbol="QQQ", window=60,
                              initial_train=120, eval_size=40)
...
def _client_with(report_json: dict) -> TestClient:
    ...
    _sqlite_register(
        ...
        dataset_hash="f" * 64, report_json=report_json,
...
def test_latest_serves_range_block_when_present():
    ...
    range_block = {
        "target": "realized_range_pct", "model": RANGE_MODEL,
        "cone": {"p10": 0.006, "p50": 0.009, "p90": 0.014, "units": "log(high/low)"},
        "coverage_80": 0.88, "crps": 0.006, "badge": "amber", "calibrated": False,
    }
    client = _client_with({
        "tft_lite_daily_qqq": {"coverage_80": 0.79}, "range": range_block})
